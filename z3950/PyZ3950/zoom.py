...
# from z3950.PyZ3950 import grs1
...
class Connection(_AttrCheck, _ErrHdlr):
    ...
    def connect(self):
        ...
        # else:
        #     options = []
        # initkw['ConnectionError'] = ConnectionError  # Not needed, defined and raised in z3950
        # initkw['ProtocolError'] = ProtocolError  # Not needed, defined and raised in z3950
        # initkw['UnexpectedCloseError'] = UnexpectedCloseError  # Not needed, defined and raised in z3950
...
class ResultSet(_AttrCheck, _ErrHdlr):
    ...
    def __getitem__(self, i):
        ...
        if type(i) == int:
        ...
        elif type(i) == slice:  # extracted from the __getslice__ method which should be part of __getitem__
            if i.start is None:
                j = 0
            else:
                j = self._pin(i.start)
            if i.stop is None:
                ...
                k = self._pin(i.stop)
            for l in range(j, k):
                self._ensure_present(l)
            if len(self._records) == 0:  # XXX is this right?
                return []
            return self._records[self.preferredRecordSyntax][self.elementSetName][j:k]
    ...
    #     if len(self._records) == 0:  # XXX is this right?
    #         return []
    #     return self._records[self.preferredRecordSyntax][self.elementSetName][i:j]
    ...
    def _get_rec(self, i):
        return self._records[self.preferredRecordSyntax][self.elementSetName][int(i)]
    ...
    def _extract_recs(self, records, lbound):
        ...
        for i, r in enumerate(recs):
            ...
            if typ == 'surrogateDiagnostic':
                ...
                type, dat = data.encoding
                if oid == oids.Z3950_RECSYN_USMARC_ov and type != 'octet-aligned':
                    raise ProtocolError("Weird record EXTERNAL MARC type: " + type)
                marc8 = False
                if dat[9] != 'a':
                    marc8 = True
                rec = Record(data=dat.encode("utf-8"), marc8=marc8)
            else:
                raise ProtocolError("Bad typ %s data %s" %
                                    (str(typ), str(data)))
            self._records[self.preferredRecordSyntax][self.elementSetName][lbound + i] = rec
